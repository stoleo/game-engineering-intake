...
function stepFilled(step) {
  ...
  if (!crit.length) return step.fields.some(function (f) { return (state.answers[f[0]] || "").trim(); });
  return crit.every(function (f) { return (state.answers[f[0]] || "").trim(); });
}
function go(i) {
...
function startScreen() {
  return '<div class="kicker">What this is</div>' +
    "<h1>Turn a messy game into a plan you can ship</h1>" +
    '<p class="lede">This is not a review tool and it is not a feature brainstorm. It diagnoses the current build, scores the real machine against a 10/10 for this game, then gives a phased plan of changes that actually move the score.</p>' +
    '<div class="grid3">' +
...
function renderScreen() {
  ...
  if (step.kind === "generate") {
    var ready = readiness();
    var missing = ready.missing.map(function (m) { return '<span class="chip bad">' + m + "</span>"; }).join("") || '<span class="chip good">Critical fields present</span>';
...
async function ingestFiles(list) {
  for (const file of [...list]) {
    var rec = { name: file.name, size: file.size, type: file.type, at: new Date().toISOString(), text: "", note: "" };
...
async function loadPartB() {
  try {
    var res = await fetch("PROMPT.md");
    PART_B_CACHE = (await res.text()).trim();
  } catch (e) { PART_B_CACHE = ""; }
}
...
function readiness() {
  ...
  var missing = need.filter(function (x) { return !(state.answers[x[0]] || "").trim(); }).map(function (x) { return x[1]; });
  var score = Math.round(((need.length - missing.length) / need.length) * 100);
  var note = "Thin packet. Expect Phase 0 and Low confidence.";
...
function updateMeter() {
  ...
  var filled = fields.filter(function (id) { return (state.answers[id] || "").trim(); }).length;
